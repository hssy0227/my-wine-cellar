...
## 5단계: 원천을 Supabase로 (2026-08)

seed CSV를 사람이 편집하던 구조에서, Supabase를 원천으로 삼고 웹에서 편집하는
구조로 옮겼다. 상세는 `docs/SUPABASE.md`, 판단 근거는 `docs/DECISIONS.md` D12~D14.

이관의 안전장치는 **바이트 동일성**이었다. Supabase에서 되읽은 seed CSV가 git의
것과 바이트 단위로 같고, 그것으로 빌드한 dist도 커밋본과 바이트 단위로 같아야
통과시켰다(`scripts/verify_parity.py`). 이 하나로 행 순서·컬럼 순서·BOM·따옴표
처리·빈값 표현·별칭 원문 철자 14건이 전부 커버된다.

실행 전에 로컬 PostgreSQL 16으로 미리 검증했고, 거기서 설계 결함 하나를 잡았다:
초안의 `name_ko ~ '[가-힣]'` CHECK를 그대로 넣었으면 `1865`(칠레 브랜드) 때문에
이관이 첫 행부터 실패했을 것이다. 한글 없는 한글명은 audit이 이미 "확인 필요"로
분류한 정당한 데이터라, DB는 받고 API가 경고만 띄우도록 바꿨다.

> **3단계와 달리 이번엔 되돌릴 수 있다.** 커밋 `3812462`가 이관 직전 상태이고,
> git에 완전한 사본이 있어 Supabase를 삭제해도 데이터를 잃지 않는다.
